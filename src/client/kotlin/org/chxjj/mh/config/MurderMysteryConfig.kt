...
object MurderMysteryConfig {
    ...
    private fun floatOption(nameKey: String, default: Float, getter: () -> Float, setter: (Float) -> Unit): Option<Float> {
    ...
    fun generateConfigScreen(parent: Screen?): Screen {
        ...
        val colorCategory = ConfigCategory.createBuilder()
            ...
            .option(floatOption("mh.config.colors.murdererRed", 203f, { murdererRed }, { murdererRed = it }))
            .option(floatOption("mh.config.colors.murdererGreen", 9f, { murdererGreen }, { murdererGreen = it }))
            .option(floatOption("mh.config.colors.murdererBlue", 9f, { murdererBlue }, { murdererBlue = it }))
            .option(floatOption("mh.config.colors.detectiveRed", 0f, { detectiveRed }, { detectiveRed = it }))
            .option(floatOption("mh.config.colors.detectiveGreen", 144f, { detectiveGreen }, { detectiveGreen = it }))
            .option(floatOption("mh.config.colors.detectiveBlue", 255f, { detectiveBlue }, { detectiveBlue = it }))
            .build()
